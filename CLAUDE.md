...
- `opening_amount` mặc định lấy từ số thực đếm của ca liền trước, nhưng **cho phép
  ghi đè** — mọi lần ghi đè khác số hệ thống tính đều ghi `audit_logs` với hành động
  `OVERRIDE_OPENING_AMOUNT` kèm cả hai con số
...
⚠️ **"Ngoài giờ" là trạng thái hiển thị, KHÔNG phải chỗ để tiền rơi vào.** Đơn
bán ngoài giờ vẫn được gán vào **ca gần nhất** — trước 7:30 vào ca sáng, sau
21:00 vào ca tối — vì tiền đó nằm trong két khi ca tối bàn giao nên phải thuộc
ca tối.

Hai hàm khác nhau, đừng dùng lẫn: `ShiftService.currentShift()` trả rỗng khi
ngoài giờ và chỉ dùng để hiện badge trên POS; `ShiftService.shiftForRevenue()`
không bao giờ rỗng và là hàm dùng khi ghi đơn.

Trước khi tách hai hàm này, đơn ngoài giờ có `shift_type_id = null` và **không
xuất hiện ở phiếu bàn giao nào** — ca đó hiện thừa tiền mà không ai hiểu vì sao,
tức phiếu đối soát tự tạo ra tín hiệu giả.
...
là lệch cả chuỗi, và tiền có thể ra vào két ngoài giờ bàn giao.

Ai cũng ghi đè được, không riêng ADMIN — cùng cách quán xử giảm giá: chỗ dựa là
**dấu vết** chứ không phải chặn trước. Ghi đè khác số hệ thống tính thì ghi audit
`OVERRIDE_OPENING_AMOUNT` kèm cả số tính ra lẫn số đã nhập. Chỉ ghi khi **khác** —
ghi mọi lần lưu thì những lần ghi đè thật bị chôn lẫn trong bản ghi vô nghĩa.

Sửa phiếu mà không gửi tiền đầu ca thì **giữ nguyên số cũ**, không tính lại. Khác
dòng POS (POS tính lại vì lý do sửa phiếu thường là vừa huỷ một đơn) — tiền đầu ca
đã là con số của thời điểm nhận ca, tự đổi nó trong một lần sửa không liên quan là
làm sai biên bản.
