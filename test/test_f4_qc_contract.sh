...
c=$(req POST /qc-tests '{"doseType":"DOUBLE","note":"phien test dau ca","tests":[
...
c=$(req POST /qc-tests "{\"doseType\":\"SINGLE\",\"tests\":[{\"stockImportId\":\"$LO\",\"doseGram\":11.5,\"yieldGram\":23,\"extractionSeconds\":34,\"grindSetting\":\"11.5\",\"boilerTempC\":128,\"acidity\":4,\"body\":4,\"sweetness\":4,\"passed\":true}]}")
eq "Tao phien Single Arabica DAT co lo" 201 "$c"
c=$(req GET /qc-tests/profile); eq "GET /qc-tests/profile" 200 "$c"
CELL='.data[] | select(.beanType=="ARABICA" and .doseType=="SINGLE")'
...
c=$(req POST /qc-tests "{\"doseType\":\"DOUBLE\",\"tests\":[{\"boilerTempC\":150,\"acidity\":3,\"body\":3,\"sweetness\":3,\"passed\":true}]}")
eq "Nhiet do noi hoi 150 -> chap nhan" 201 "$c"
c=$(req POST /qc-tests "{\"doseType\":\"DOUBLE\",\"tests\":[{\"boilerTempC\":250,\"acidity\":3,\"body\":3,\"sweetness\":3,\"passed\":true}]}")
eq "Nhiet do 250 -> chan (typo)" 400 "$c"

# Lan DAT moi hon de len profile (11.5->24), khong dat thi khong len
c=$(req POST /qc-tests "{\"doseType\":\"SINGLE\",\"tests\":[{\"stockImportId\":\"$LO\",\"doseGram\":12,\"yieldGram\":24,\"extractionSeconds\":30,\"grindSetting\":\"12\",\"acidity\":5,\"body\":5,\"sweetness\":5,\"passed\":true}]}")
